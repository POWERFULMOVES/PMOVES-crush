...
func (b *bashTool) bashDescription() string {
	...
	if generatedWith {
		prAttribution = "🤖 Generated with PMOVES-Crush"
	}

	if generatedWith || coAuthoredBy {
		var attributionParts []string
		if generatedWith {
			attributionParts = append(attributionParts, "🤖 Generated with PMOVES-Crush")
		}
		if coAuthoredBy {
			attributionParts = append(attributionParts, "Co-Authored-By: PMOVES Agent <agent@pmoves.ai>")
		}
